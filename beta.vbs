...
Function showArr2(arrPrintLoc, personNum) 'show arr
	'* 
    For x = 1 To 4
    	For y = 1 To 4
...
Function timeSum(arrPrintLoc, personNum)
	For i = 1 To personNum	
    	...
    	For j = printRowRanInit To arrPrintLoc(3,i)
    		...
    		Cells(printRowRanInit + rowRanFin - rowRanInit, arrPrintLoc(2,i)+1).Value = timeSum
    	Next
    	arrPrintLoc(4,i) = Cells(printRowRanInit + rowRanFin - rowRanInit, arrPrintLoc(2,i)+1).Value  '왜 바로 timeSum을 넣으면 값이 이상하게 들어가는 지 모르겠는데 이렇게 한 번 경유하면 잘 됨
...
Sub TimetableWizard()
    ...
    For i = 1 To 100
	    ...
	    Call showArr2(arrPrintLoc, personNum)
	    Cells(30, 1).Value = minWho(arrPrintLoc, personNum)
	    maxVal = 0
	   	Call maxfn(arr, maxVal, maxIdx)
	   	Cells(arrPrintLoc(3,j), arrPrintLoc(2,j)).Value = arr(element, maxIdx)
	  	Cells(arrPrintLoc(3,j), arrPrintLoc(2,j)+1).Value = maxVal
